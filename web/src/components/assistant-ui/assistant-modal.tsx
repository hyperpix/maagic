...
export const AssistantModal: FC<{ clearChat?: () => void }> = ({ clearChat }) => {
  return (
...
>(({ "data-state": state, ...rest }, ref) => {
  ...
  return (
    <TooltipIconButton
      ...
      side="left"
      {...rest}
      className="aui-modal-button size-full rounded-full bg-yellow-500 hover:bg-yellow-600 shadow transition-transform hover:scale-110 active:scale-90"
      ref={ref}
    >
      <MessageCircle
        data-state={state}
        className="aui-modal-button-closed-icon absolute size-6 text-white transition-all data-[state=closed]:rotate-0 data-[state=open]:rotate-90 data-[state=closed]:scale-100 data-[state=open]:scale-0"
      />
